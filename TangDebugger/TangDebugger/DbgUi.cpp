...
{
	SetConsoleCursorPosition(m_hStdOut, COORD{ (short)x , (short)y });
	SetConsoleTextAttribute(m_hStdOut, (short)color);
	printf("%s", pszStr);
}
	...
	list<BPObject*>::const_iterator endItr)
{
	...
	for (; beginItr != endItr; ++beginItr)
	{
		...
		{
			wchar_t wstr[] = L"硬件读断点";
			wprintf(L"%s    |", wstr);
		}
			break;
		case breakpointType_hard_w:
		{
			wchar_t wstr[] = L"硬件写断点";
			wprintf(L"%s    |", wstr);
		}
			break;
		case breakpointType_hard_e:
		{
			wchar_t wstr[] = L"硬件执行断点";
			wprintf(L"%s    |", wstr);
		}
			break;
		case breakpointType_hard_rw:
		{
			wchar_t wstr[] = L"硬件读写断点";
			wprintf(L"%s    |", wstr);
		...
		{
			wchar_t wstr[] = L"内存读断点";
			wprintf(L"%s    |", wstr);
		}
			break;
		case breakpointType_acc_w:
		{
			wchar_t wstr[] = L"内存写断点";
			wprintf(L"%s    |", wstr);
		}
			break;
		case breakpointType_acc_e:
		{
			wchar_t wstr[] = L"内存执行断点";
			wprintf(L"%s    |", wstr);
		}
			break;
		...
		printf(" %s\n", pCondition == nullptr ? "none" : pCondition);
	}
	printf("------+------------+----------------+---------------\n");
}
